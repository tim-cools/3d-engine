...
import {addTrianglesBySegments} from "./addFacesBySegments"

export type SubtractSegmentsResult = {points: Point[], segments: Segment[], faces: Face[]}

export function subtractSegments(models: SubtractModels, log: Logger): SubtractSegmentsResult {

  const master: Model = models.master
  const subtract: SpaceModel = models.subtract
  const intersections: Point[] = []
  const intersectionsEnds: Point[] = []
  const segments: Segment[] = []
  const faces: Face[] = []

  function result()
  {
    return {segments: segments, points: [], faces: faces}
  }

  function addMasterSegments() {
    log.logLine(`- master.segments: ${master.segments.length}`)
    for (const segment of master.segments) {
      addMasterSegment(segment)
    }
  }

  function addMasterSegment(segment: Segment) {
    ...
    if (beginInSubtract && endInSubtract) {
      ...
      const partials = getPartials(segment)
      if (partials != nothing) {
  ...
  function addSubtractSegments() {
    log.logLine(`- subtract.segments: ${subtract.segments.length}`)
    for (const segment of subtract.segments) {
      addSubtractSegment(segment)
    }
  }

  function addSubtractSegment(segment: Segment) {
    ...
    if (beginInMaster && endInMaster) {
      ...
      const partials = partialMaster(master, segment)
      if (partials != nothing) {
        partials.map(partial => segments.push(partial))
      } else {
        segments.push(segment.disabled(true))
  ...
  function connectIntersectionsPoints() {

  }

  function getPartials(segment: Segment): Segment[] | Nothing {
    for (const face of subtract.faces) {
      for (const triangle of face.triangles) {
        ...
        if (intersection.type == IntersectionType.Point) {
          ...
          return partialSubtractIntersectionSegments(segment, intersection.segment)
        }
      }
    }
    return nothing
  ...
  function partialMaster(model: Model, segment: Segment): Segment[] | Nothing {
  ...
  function partialSegmentsPointIntersection(subtract: CanContainPoint, segment: Segment, intersectionPoint: Point, invert: boolean) {
    ...
    intersectionsEnds.push(intersectionPoint)
    return result
  }

  function partialSubtractIntersectionSegments(segment: Segment, intersectionSegment: Segment) {
  ...
  addMasterSegments()
  addSubtractSegments()
  addIntersectionsPath()
  connectIntersectionsPoints()

  addTrianglesBySegments(segments, faces)
